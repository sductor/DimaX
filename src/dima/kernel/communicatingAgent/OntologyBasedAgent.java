...
public abstract class OntologyBasedAgent extends BasicCommunicatingAgent {
	...
	public OntologyBasedAgent(final AgentIdentifier id){
		...
		try{
			...
			try{
				...
				OntologyBasedAgent.ontoModel = new DAMLModelImpl();
				OntologyBasedAgent.ontoModel.read(source.getCharacterStream(),
	...
	public Iterator getAllInstances(){
		return OntologyBasedAgent.ontoModel.listDAMLInstances();
	}
	...
	public Iterator getAllProperties(){
		return OntologyBasedAgent.ontoModel.listDAMLProperties();
	}
	/**
	 * @return java.util.Iterator
	 */
	...
	public Iterator getAllClasses(){
		return OntologyBasedAgent.ontoModel.listDAMLClasses();
	}
	...
	public boolean addInstance(final String className, final String resource, final String predicat, final Object value) throws RDFException {
		...
		final DAMLClass upper = (DAMLClass) OntologyBasedAgent.ontoModel.getDAMLValue(className);

		// here we test if this class exist really in the ontology

		if(this.hasClass(upper))
		{
			final DAMLInstance instance = OntologyBasedAgent.ontoModel.createDAMLInstance(upper,resource);
			final DAMLProperty property = (DAMLProperty) OntologyBasedAgent.ontoModel.getDAMLValue(predicat);
	...
	public boolean addPropertyToInstance(final String resource, final String predicat, final Object value) throws RDFException {
		...
		final DAMLInstance instance = (DAMLInstance) OntologyBasedAgent.ontoModel.getDAMLValue(resource);
	...
	public void changeProperty(final String resource, final String predicat, final Object newValue) throws RDFException {
		...
		final DAMLInstance instance = (DAMLInstance) OntologyBasedAgent.ontoModel.getDAMLValue(this.refKb+"#"+resource);
		final DAMLProperty property = (DAMLProperty) OntologyBasedAgent.ontoModel.getDAMLValue(this.refKb+"#"+predicat);
	...
	public boolean hasInstance(final String resource, final String predicat, final Object value) throws RDFException {
		...
		final Iterator it = OntologyBasedAgent.ontoModel.listDAMLInstances();
	...
	public ReasoningStepIterator askReasoner(final String queryKif, final String premise)
	{
		...
		final DamlReasoningContext drc = this.getDAMLReasoningContext(OntologyBasedAgent.ontoModel);
	...
	{
		final Map bindings = OntologyBasedAgent.getBindings(rs);
		final Map retBindings = new HashMap(bindings.size());
		final Set topLevelVars = OntologyBasedAgent.getQueryVariables(rs);
