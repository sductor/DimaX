...
extends Protocol<SimpleNegotiatingAgent<ActionSpec, State, Contract>> {
	...
	public boolean negotiationAsInitiatorHasStarted() {
		return !this.contracts.getAllInitiatorContracts().isEmpty();
	}
	...
	public void initiateNegotiation() {
		if (this.isActive() && !this.negotiationAsInitiatorHasStarted()
			...
			try {
				...
				for (final Contract c : cs){
					...
					this.sendMessage(
							...
							new SimpleContractEnvellope(Performative.Propose, c));
	...
	protected abstract void answerAccepted(ArrayList<Contract> toAccept);

	protected abstract void answerRejected(ArrayList<Contract> toReject);

	protected abstract void putOnWait(ArrayList<Contract> toPutOnWait);
	...
	protected void acceptContract(final Collection<Contract> contracts, final Receivers receivers) {
		...
		if (!contracts.isEmpty())this.getMyAgent().logMonologue(
	...
	protected void confirmContract(final Collection<Contract> contracts, final Receivers receivers) {
		...
		try {
			for (final ActionSpec s : ReallocationContract.getResultingAllocation(contracts))
	...
	protected void cancelContract(final Collection<Contract> contracts, final Receivers receivers) {
		...
		if (!contracts.isEmpty())this.getMyAgent().logMonologue("**************> I cancel!"+contracts,
				AbstractCommunicationProtocol.log_negotiationStep);
	...
	protected void receiveProposal(final SimpleContractEnvellope delta)  {
	...
	protected void receiveConfirm(final SimpleContractAnswer m) {
		...
		this.getMyAgent().logMonologue("I'll apply proposal "+contract,AbstractCommunicationProtocol.log_negotiationStep);//+"\n"+contracts);
		this.getMyAgent().getMyInformation().add(m.getSpec());
	...
	public class SimpleContractAnswer extends FipaACLMessage {
		...
		public SimpleContractAnswer(final Performative p,
			...
			try {
				AbstractCommunicationProtocol.this.getContracts().getContract(id).setSpecification(this.s);
			} catch (final UnknownContractException e) {
				AbstractCommunicationProtocol.this.signalException("impossible!!", e);
			}
		}
