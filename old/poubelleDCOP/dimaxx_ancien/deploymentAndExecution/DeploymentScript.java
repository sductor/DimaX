...
import dimaxx.deploymentAndExecution.Executor.ErrorOnProcessExecutionException;
import dimaxx.deploymentAndExecution.Executor.WrongOSException;
...
public class DeploymentScript {
	...
	public DeploymentScript(final File machineFile) throws JDOMException,
	IOException {
	...
	public DeploymentScript(final String listeMachines) throws JDOMException, IOException {
	...
	protected RemoteHostExecutor addRemoteHost(final RemoteHostExecutor remoteHost) {
		this.hostsOfNetworks.add(remoteHost.groupID, remoteHost);
		this.networksOfHosts.put(remoteHost, remoteHost.groupID);
		return remoteHost;
	}

	protected boolean removeRemoteHost(final RemoteHostExecutor host){
		this.hostsOfNetworks.get(host.groupID).remove(host);
	...
	public void activateSelfDestruction(){
		new NetworkExitingShutdownHook(this.getAllHosts()).activate();
	}
