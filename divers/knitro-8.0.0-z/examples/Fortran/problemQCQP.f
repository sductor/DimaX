...
C  COMPUTE THE HESSIAN OF THE LAGRANGIAN AT x AND lambda.
      ...
      subroutine compute_h (x, lambda, hess)
      ...
      hess(1) =  2.0d0*(lambda(2) - 1.0d0)
      hess(2) = -1.0d0
      hess(3) = -1.0d0
      hess(4) =  2.0d0*(lambda(2) - 2.0d0)
      hess(5) =  2.0d0*(lambda(2) - 1.0d0)
...
C  RETURN THE RESULT IN vector.
      ...
      subroutine compute_hv (n, x, lambda, vector)
      ...
      tmp(1) = 2.0d0*(lambda(2) - 1.0d0)*vector(1)
     $         - vector(2) - vector(3)
      tmp(2) = -vector(1) + 2.0d0*(lambda(2) - 2.0d0)*vector(2)
      tmp(3) = -vector(1) + 2.0d0*(lambda(2) - 1.0d0)*vector(3)
