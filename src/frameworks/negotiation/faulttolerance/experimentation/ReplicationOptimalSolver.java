...
public class ReplicationOptimalSolver extends BasicAgentModule<ReplicationLaborantin> {
	...
	public ReplicationOptimalSolver(final ReplicationLaborantin l) {
		super(l);
	...
	private ReplicationOptimalSolver(final ReplicationExperimentationParameters p) {
		super();
		...
		for (int j = 0; j < this.nbHosts; j++){
			...
			for (int i = 0; i < this.nbAgents; i++){
				...
				if (s.getVar(this.hostsMatrix[j][i]).getVal()==1){
					final AgentIdentifier agId = this.ags[i].getMyAgentIdentifier();
					alloc+=agId+" , ";
					final boolean allocOk = ReplicationInstanceGraph.allocateAgents(this.getMyAgent(), agId, hId, finalRepAlloc, finalHostAlloc);
					assert allocOk;
	...
	private void generateConstant(){
		...
		for (int i = 0; i < this.nbAgents; i++){
			this.agentCriticity[i]=Choco.constant(ReplicationOptimalSolver.asInt(this.ags[i].getMyCriticity(),true));
			this.repProcCharge[i]=ReplicationOptimalSolver.asInt(this.ags[i].getMyProcCharge(),false);
			this.repMemCharge[i]=ReplicationOptimalSolver.asInt(this.ags[i].getMyMemCharge(),false);
		...
		for (int i = 0; i < this.nbHosts; i++){
			this.hostLambda[i]=ReplicationOptimalSolver.asInt(this.hs[i].getLambda(),true);
			this.hostProcCap[i]=ReplicationOptimalSolver.asInt(this.hs[i].getProcChargeMax(),false);
			this.hostMemCap[i]=ReplicationOptimalSolver.asInt(this.hs[i].getMemChargeMax(),false);
	...
	public static void main(final String[] args) throws IfailedException, CompetenceException, NotEnoughMachinesException{
		...
		final ReplicationOptimalSolver ros = new ReplicationOptimalSolver(p);
		ros.solve();
	}
}
