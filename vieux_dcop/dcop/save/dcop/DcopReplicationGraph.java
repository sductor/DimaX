...
public abstract class DcopReplicationGraph extends DcopAbstractGraph {
	...
	public DcopReplicationGraph(
		...
		for (AgentState a : rig.getAgentStates()) {
			...
			Variable v = new Variable(
					...
					this);
			assert !varMap.containsKey(identifierToInt(id));
		...
		for (AgentState a : rig.getAgentStates()) {
			...
			for (ResourceIdentifier idHost : rig.getAccessibleHost(idAgent)){
				Variable agentVar = varMap.get(identifierToInt(idAgent));
				Variable hostVar = varMap.get(identifierToInt(idHost));
	...
	private Integer identifierToInt(AgentIdentifier id){
