...
public class CopyOfReplicationExperimentationGenerator extends ReplicationExperimentationParameters{
	...
	//problem
	private int[] seedDomain = new int[]{5564864,646464,94864};
	
	private int[] nbAgentDomain = new int[]{0,100,250,500,1000,5000};
	private int[] nbHostDomain = new int[]{24};
	
	private double[] hostCapacity = new double[]{0.3,0.6, -cst};
	private double[] graphDensityDomain = new double[]{0.2,1.};
	...
	private String[] selectDomain = new String[]{
	...
	//solveur
	private double[] kDomain= new double[]{0.3,0.6,-100,-500};
	
	//Social
	private double[][] alphaDomain= new double[][]{new double[]{0.3,0.6}, new double[]{0.5,0.75}, new double[]{0.25,0.5}};
	private double[] kOpinionDomain= new double[]{0.3,0.6,0.1};
	...
	CopyOfReplicationExperimentationGenerator(int nbAgents, int nbHosts,
...
//	static boolean varyFault=false;
	...
	static ReplicationExperimentationParameters getDefaultParameters() {
		return new ReplicationExperimentationParameters(
				CopyOfReplicationExperimentationGenerator.startingNbAgents,
				CopyOfReplicationExperimentationGenerator.startingNbHosts,
				...
				CopyOfReplicationExperimentationGenerator.doubleParameters.get(1),//criticity mean
	...
	public static String getProtocolId() {
		return ExperimentationParameters._maxSimulationTime / 1000
				+ "secs"
				+ (CopyOfReplicationExperimentationGenerator.varyAgentSelection==true?"varyAgentSelection":"")
				+ (CopyOfReplicationExperimentationGenerator.varyHostSelection?"varyHostSelection":"")
				+ (CopyOfReplicationExperimentationGenerator.varyProtocol?"varyProtocol":"")
				+ (CopyOfReplicationExperimentationGenerator.varyHostDispo?"varyHostDispo":"")
				+ (CopyOfReplicationExperimentationGenerator.varyHostSelection?"varyHostSelection":"")
				+ (CopyOfReplicationExperimentationGenerator.varyOptimizers?"varyOptimizers":"")
				+ (CopyOfReplicationExperimentationGenerator.varyAccessibleHost?"varyAccessibleHost":"")
				+ (CopyOfReplicationExperimentationGenerator.varyAgentLoad?"varyAgentLoad":"")
				+ (CopyOfReplicationExperimentationGenerator.varyHostCapacity?"varyHostCapacity":"");
	}
	...
	public LinkedList<ExperimentationParameters<ReplicationLaborantin>> generateSimulation() {
		...
		//		f.mkdirs();
		new File(LogService.getMyPath()+"result_"+CopyOfReplicationExperimentationGenerator.getProtocolId()+"/").mkdirs();
		Collection<ReplicationExperimentationParameters> simuToLaunch =
				new HashSet<ReplicationExperimentationParameters>();
		simuToLaunch.add(CopyOfReplicationExperimentationGenerator.getDefaultParameters());
		if (CopyOfReplicationExperimentationGenerator.varyAgents) {
			simuToLaunch = this.varyAgents(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyHosts) {
			simuToLaunch = this.varyHosts(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyAccessibleHost) {
			simuToLaunch = this.varyAccessibleHost(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyHostDispo) {
			simuToLaunch = this.varyHostDispo(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyHostFaultDispersion) {
			simuToLaunch = this.varyHostFaultDispersion(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyAgentLoad) {
			simuToLaunch = this.varyAgentLoad(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyAgentLoadDispersion) {
			simuToLaunch = this.varyAgentLoadDispersion(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyHostCapacity) {
			simuToLaunch = this.varyHostCapacity(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyHostCapacityDispersion) {
			simuToLaunch = this.varyHostCapacityDispersion(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyAgentCriticity) {
			simuToLaunch = this.varyAgentCriticity(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyAgentCriticityDispersion) {
			simuToLaunch = this.varyAgentCriticityDispersion(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyAgentSelection) {
			simuToLaunch = this.varyAgentSelection(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyHostSelection) {
			simuToLaunch = this.varyHostSelection(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyOptimizers) {
			simuToLaunch = this.varyOptimizers(simuToLaunch);
		}
		if (CopyOfReplicationExperimentationGenerator.varyProtocol) {
...
//		}
	...
	public Integer getMaxNumberOfAgent(final HostIdentifier id) {
		return new Integer((int) this.nbSimuPerMAchine*
				//				(nbAgents + nbHosts)+1);
				(CopyOfReplicationExperimentationGenerator.startingNbAgents + CopyOfReplicationExperimentationGenerator.startingNbHosts)+1);
	...
	private Collection<ReplicationExperimentationParameters> varyProtocol(final Collection<ReplicationExperimentationParameters> exps){
		...
		for (final ReplicationExperimentationParameters p : exps) {
			for (final String v : CopyOfReplicationExperimentationGenerator.protos){
	...
	private Collection<ReplicationExperimentationParameters> varyAgentSelection(final Collection<ReplicationExperimentationParameters> exps){
		final Collection<ReplicationExperimentationParameters> result=new HashSet<ReplicationExperimentationParameters>();
		for (final ReplicationExperimentationParameters p : exps) {
			for (final String v : CopyOfReplicationExperimentationGenerator.select){
				final ReplicationExperimentationParameters n =  p.clone();
	...
	private Collection<ReplicationExperimentationParameters> varyOptimizers(final Collection<ReplicationExperimentationParameters> exps){
		final Collection<ReplicationExperimentationParameters> result=new HashSet<ReplicationExperimentationParameters>();
		for (final ReplicationExperimentationParameters p : exps) {
			for (final SocialChoiceType v : CopyOfReplicationExperimentationGenerator.welfare){
				final ReplicationExperimentationParameters n =  p.clone();
				n._socialWelfare=v;
				result.add(n);
			}
	...
	private Collection<ReplicationExperimentationParameters> varyAgents(final Collection<ReplicationExperimentationParameters> exps){
		final Collection<ReplicationExperimentationParameters> result=new HashSet<ReplicationExperimentationParameters>();
		for (final ReplicationExperimentationParameters p : exps) {
			for (final Double v : CopyOfReplicationExperimentationGenerator.doubleParameters6){
				final ReplicationExperimentationParameters n =  p.clone();
				n.nbAgents=(int)(v*CopyOfReplicationExperimentationGenerator.startingNbAgents);
	...
	private Collection<ReplicationExperimentationParameters> varyHosts(final Collection<ReplicationExperimentationParameters> exps){
		...
		for (final ReplicationExperimentationParameters p : exps) {
			for (final Double v : CopyOfReplicationExperimentationGenerator.doubleParameters6){
				final ReplicationExperimentationParameters n =  p.clone();
				n.nbHosts=(int)(v*CopyOfReplicationExperimentationGenerator.startingNbHosts);
			}
		}
		return result;
	}
	private Collection<ReplicationExperimentationParameters> varyAccessibleHost(final Collection<ReplicationExperimentationParameters> exps){
		final Collection<ReplicationExperimentationParameters> result=new HashSet<ReplicationExperimentationParameters>();
		for (final ReplicationExperimentationParameters p : exps) {
			for (final Double v : CopyOfReplicationExperimentationGenerator.doubleParameters4){
				final ReplicationExperimentationParameters n =  p.clone();
				n.setkAccessible(v);
				result.add(n);
			}
	...
	private Collection<ReplicationExperimentationParameters> varyHostDispo(final Collection<ReplicationExperimentationParameters> exps){
		final Collection<ReplicationExperimentationParameters> result=new HashSet<ReplicationExperimentationParameters>();
		for (final ReplicationExperimentationParameters p : exps) {
			for (final Double v : CopyOfReplicationExperimentationGenerator.doubleParameters5){
				final ReplicationExperimentationParameters n =  p.clone();
	...
	private Collection<ReplicationExperimentationParameters> varyHostFaultDispersion(final Collection<ReplicationExperimentationParameters> exps){
		final Collection<ReplicationExperimentationParameters> result=new HashSet<ReplicationExperimentationParameters>();
		for (final ReplicationExperimentationParameters p : exps) {
			for (final DispersionSymbolicValue v : CopyOfReplicationExperimentationGenerator.dispersion){
				final ReplicationExperimentationParameters n = p.clone();
	...
	private Collection<ReplicationExperimentationParameters> varyHostCapacity(final Collection<ReplicationExperimentationParameters> exps){
		...
		for (final ReplicationExperimentationParameters p : exps) {
			for (final Double v : capacitiesList){
				...
				final ReplicationExperimentationParameters n2 = p.clone();
				n2.hostCapacityMean=(double)CopyOfReplicationExperimentationGenerator.startingNbAgents/(double)CopyOfReplicationExperimentationGenerator.startingNbHosts;
	...
	private Collection<ReplicationExperimentationParameters> varyAgentCriticity(final Collection<ReplicationExperimentationParameters> exps){
		final Collection<ReplicationExperimentationParameters> result=new HashSet<ReplicationExperimentationParameters>();
		for (final ReplicationExperimentationParameters p : exps) {
			for (final Double v : CopyOfReplicationExperimentationGenerator.doubleParameters){
				final ReplicationExperimentationParameters n = p.clone();
...
//	}
	private Collection<ReplicationExperimentationParameters> varyDynamicCriticity(
			final Collection<ReplicationExperimentationParameters> exps) {
		assert CopyOfReplicationExperimentationGenerator.dynamicCriticityKey>=-1 && CopyOfReplicationExperimentationGenerator.dynamicCriticityKey<=1;
		final Collection<ReplicationExperimentationParameters> result=new HashSet<ReplicationExperimentationParameters>();
		for (final ReplicationExperimentationParameters p : exps) {
			if (CopyOfReplicationExperimentationGenerator.dynamicCriticityKey==-1){
				p.dynamicCriticity=false;
				result.add(p);
			} else if (CopyOfReplicationExperimentationGenerator.dynamicCriticityKey==1){
