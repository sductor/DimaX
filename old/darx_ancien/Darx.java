...
public class Darx extends UnicastRemoteObject implements DarxServer {
			...
			if (parameters[i].compareTo("-ns") == 0) {
				...
				System.out
						.println("Incorrect use of DARX server startup command:"
								...
								+ "[-p <port_number>]'");
				Collection<String> yo = Arrays.asList(parameters);
				System.out
	...
	public RemoteTask startTask(final DarxTask task) throws RemoteException {
		...
		try {
			...
			System.err.println("Error on "+generic_task_name+" activation : " + new_rep_info.textifyDarxName()
					+ " is not bound ?!?!");
		}
	...
	public RemoteTask findTask(final String task_name) throws RemoteException {
		...
		try {
			...
			try {
				ts_handle = task_info.getTaskShellHandle(); //System.out.println(" ts_handle : "+ts_handle);
				if (ts_handle==null)
					next=true;
			} catch (final Exception e) {
				next = true;// System.out.println(" ts_handle : "+ts_handle);
			}
			...
			System.out.println(" ici ");throw e;
