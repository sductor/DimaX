package frameworks.faulttolerance.dcop.api;
...
public class DCOPExperimentationParameters extends ExperimentationParameters<Laborantin>{
	...
	public static DcopAbstractGraph g;
	...
	public void initiateParameters() {
		...
		DCOPExperimentationParameters.g = new DcopAbstractGraph(this.filename);
	...
	public boolean verification(){
		...
		for (final Constraint c : DCOPExperimentationParameters.g.conList){
			assert DcopAbstractGraph.constraintExist(DCOPExperimentationParameters.g, c.first.id, c.second.id);
		...
		for (final BasicCompetentAgent a : DCOPExperimentationParameters.nodeMap.values()){
			...
			for (final Channel c : n.getIn().getChannels()){
				...
				assert DcopAbstractGraph.constraintExist(DCOPExperimentationParameters.g, c.getOwner().getIdentifier().asInt(), c.getNeighbor().asInt());
