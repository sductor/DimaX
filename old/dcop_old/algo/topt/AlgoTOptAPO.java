...
import examples.dcop_old.algo.LockingBasicAlgorithm;
...
public class AlgoTOptAPO extends LockingBasicAlgorithm {
	...
	public void initialisation(){
		...
		for (int i = 0; i < in().getSize(); i++) {
			int id =  ((Channel) in(i)).getNeighbor();
			inChannelMap.put(id, i);
		}

		for (int i = 0; i < out().getSize(); i++) {
			int id = ((Channel) out(i)).getNeighbor();
...
//				assert Graph.constraintExist(DCOPLaborantin.g, getID(), vmsg.getId()):getID()+" "+vmsg.getId();
				assert view.varMap.containsKey(vmsg.getId());
				Variable v = view.varMap.get(vmsg.getId());
				assert v != null:getID()+" "+vmsg.getId()+" "+view;
				
				ValueMsg vmsgFor = vmsg.forward();
//				assert Graph.constraintExist(DCOPLaborantin.g, getID(), vmsgFor.getId()):getID()+" "+vmsgFor.getId();
				assert view.varMap.containsKey(vmsgFor.getId());
				
				if (vmsg.ttl > 1)
					out().broadcast(vmsgFor);
