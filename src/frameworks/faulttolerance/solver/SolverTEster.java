		...
		for (int heuristic=1; heuristic<=2; heuristic++){
			...
			try {
				for (int i = 0; i < 10; i++){
					...
					KnitroAllocationSolver kas2 = new KnitroAllocationSolver(rig.getSocialWelfare(), false, true, 2, false, -1);
					kas2.myState=rig.getHostsStates().iterator().next();
					kas2.setProblem(rig);
					kas2.intialSolution=kas2.getInitialAllocAsSolution(new double[kas2.getVariableNumber()]);
					kas2.initiateSolver();
					double[] bestPossible = kas2.getBestTriviaSol(best,hostCap);
					...
					if (knitro){
						...
						kas = new KnitroAllocationSolver(rig.getSocialWelfare(), false, true, 2, false, -1);
						kas.myState=rig.getHostsStates().iterator().next();
						kas.setProblem(rig);
						kas.intialSolution=kas.getInitialAllocAsSolution(new double[kas.getVariableNumber()]);
						kas.initiateSolver();
					...
					if (metal){
						initTime = System.currentTimeMillis();
						jms = new JMetalSolver(rig.getSocialWelfare(), false, true, heuristic);
						jms.myState=rig.getHostsStates().iterator().next();
						jms.setProblem(rig);
						jms.initiateSolver();
						jms.intialSolution=jms.getInitialAllocAsSolution(new double[jms.getVariableNumber()]);
					...
					if (bbTest){
						initTime = System.currentTimeMillis();
						bb = new DcopBranchAndBoundSolver(rig.getSocialWelfare(), false, true);
						bb.myState=rig.getHostsStates().iterator().next();
						bb.setProblem(rig);
						bb.initiateSolver();
						bb.intialSolution=bb.getInitialAllocAsSolution(new double[jms.getVariableNumber()]);
