...
public class AlgoKOptOriginal extends BasicDCOPNode {
	...
	public AlgoKOptOriginal(Variable v, int kk) {
		super(v);
		k = kk;
		self.value = getMyNode().getRandom().nextInt(self.domain);
		init();
	}
					...
					gainInfoMap.put(g.id, g);
						if (gainInfoSet.size() == self.neighbors.size()) {
							...
							if (gainInfoCounter < k / 2 + 1)
								getMyNode().broadcast(new KorigGainMsg(self.id, gainInfoMap));
						}
			} else
				buffer.add(msg);
	...
	public void initialisation(){
				getMyNode().broadcast(new KorigValueMsg(self));
	}
	...
	public void main() {
			...
			if (index != -1) {
				...
				if (p instanceof BasicDCOPNode) {
					sender = ((BasicDCOPNode) p).getID();
				}
				...
				if (localInfoCounter == k / 2 + 1) {
					...
					HashSet<Integer> kgroup = new HashSet<Integer>();
								...
								while (kgroup.size() < k) {
									if (cList.isEmpty())
										break;
									int idx = cList.remove(getMyNode().getRandom().nextInt(cList
											.size()));
									kgroup.add(idx);
				...
				if (gainInfoCounter == k / 2 + 1) {
					...
					getMyNode().broadcast(new KorigCommitMsg(self.id, commitInfoMap));
				}
