...
public class AlgoKOptAPO extends LockingBasicAlgorithm {
	...
	protected void main() {
		...
		for (int i = 0; i < this.in().getSize(); i++) {
			final Program p = ((Channel) this.in(i)).getSender().getProgram();
		...
		for (int i = 0; i < this.out().getSize(); i++) {
			final Program p = ((Channel) this.out(i)).getReceiver().getProgram();
			if (p instanceof BasicAlgorithm) {
				final int id = ((BasicAlgorithm) p).getID();
		...
		while (true) {
			...
			if (index != -1) {
				...
				int sender = -1;
				final Program p = ((Channel) this.in(index)).getSender().getProgram();
				if (p instanceof BasicAlgorithm) {
