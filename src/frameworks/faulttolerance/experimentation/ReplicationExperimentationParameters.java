...
ExperimentationParameters<ReplicationLaborantin> implements Comparable {
	...
	protected Collection<RationalAgent> instanciateAgents()throws CompetenceException {
		 ...
		 *
		 */
		for (RationalAgent ag : result.values()){
			((ObservedAgent) ag).getMySelfObservingService().notifyMyInitState();
		}
	...
	int getDcopKValue(final int k){
		if (_usedProtocol.equals(NegotiationParameters.key4DcopProto2))
			return k;
		else if (_usedProtocol.equals(NegotiationParameters.key4DcopProto)){
