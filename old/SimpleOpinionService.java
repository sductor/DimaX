...
import dima.introspectionbasedagents.services.core.information.SimpleInformationDataBase;
...
import dima.introspectionbasedagents.services.modules.aggregator.FunctionalDispersionAgregator;
import dima.introspectionbasedagents.services.modules.aggregator.HeavyAggregation;
import dima.introspectionbasedagents.services.modules.aggregator.LightWeightedAverageDoubleAggregation;

/**
 * AnalysedInformation construct a personnal opinion
 * that is updated with the other received opinion to construct the global opinion
 * @author ductors
 *
 */
...
SimpleObservationService implements OpinionService {
	...
	// Fields
	//
	public static final long maxSystemExpirationTime = 60000;
	public static final AgentIdentifier globaLAgentIdentifer = new AgentName("global agent");
...
//			new HashMap<AgentIdentifier, Class<? extends Information>>();
	...
	public <Info extends Information> Opinion<Info> getGlobalOpinion(final Class<Info> myInfoType) throws NoInformationAvailableException {
		if (this.everyoneOpinion.get(myInfoType)!=null){
			try {
			...
			this.getMyAgent().signalException("requesting "+myInfoType+" no info !!!");
			throw new NoInformationAvailableException();
		}
	}
	...
	class AnalysedInformationDataBase<Info extends Information>
	extends SimpleInformationDataBase<Info> implements InformationDataBase<Info> {
		...
		TreeSet<Info> infos = new HeavyAggregation<Info>();
		HeavyAggregation<Info> dynamicity =  new HeavyAggregation<Info>();
		...
		public Info add(final Info o) {
			...
			assert minState.getNumericValue(minState) <= meanInfo.getNumericValue(meanInfo) 
					&& meanInfo.getNumericValue(meanInfo) <= maxState.getNumericValue(maxState):
						minState+"\n"+meanInfo+"\n"+maxState+"\n"+values();
		...
		public Info remove(final AgentIdentifier i) {
		...
		protected void clean() {
			for (final Information i : this.values()) {
				if (i.getUptime() - new Date().getTime() > SimpleOpinionService.maxSystemExpirationTime) {
		...
		private void updateMinMax(final Info o) {
			...
			assert (minState==null || maxState==null) || (minState.getNumericValue(minState) <=  maxState.getNumericValue(maxState)):
				minState+"\n"+maxState;
			assert (minState==null || maxState==null || meanInfo==null) || (minState.getNumericValue(minState) <= meanInfo.getNumericValue(meanInfo) 
					&& meanInfo.getNumericValue(meanInfo) <= maxState.getNumericValue(maxState)):
						minState+"\n"+meanInfo+"\n"+maxState+"\n"+this.values();
		}
	...
	InformationDataBase<Opinion<Info>> {
		...
		Opinion<Info> getGlobalOpinion() {
			...
			try {
				this.add(this.getPersonalOpinion(SimpleOpinionService.globaLAgentIdentifer));
			} catch (final NoInformationAvailableException e) {
				//Do nothing
			}
		...
		public Opinion<Info> add(final Opinion<Info> o) {
			assert (minState==null || meanInfo==null) || minState.getNumericValue(minState) <= meanInfo.getNumericValue(meanInfo):minState+"\n"+meanInfo+"\n"+maxState;
			assert (maxState==null || meanInfo==null) || meanInfo.getNumericValue(meanInfo) <= maxState.getNumericValue(maxState):minState+"\n"+meanInfo+"\n"+maxState;
			assert (maxState==null || minState==null) || minState.getNumericValue(minState) <= maxState.getNumericValue(maxState):minState+"\n"+meanInfo+"\n"+maxState;
		...
		Opinion<InformedState> {
			...
			public SimpleOpinion(
				...
				assert minState.getNumericValue(minState) <= representativeState.getNumericValue(representativeState) 
						&& representativeState.getNumericValue(representativeState) <= maxState.getNumericValue(maxState):
							this.getMaxElement()+"\n"+this.getMinElement()+"\n"+this.getRepresentativeElement(); 
			...
			public AbstractCompensativeAggregation<Information> fuse(
				...
				for (final AbstractCompensativeAggregation<? extends Information> av : averages) {
					...
					assert this.getNumericValue(fminState)<=this.getNumericValue(this.getRepresentativeElement(fallRepStates));
					assert this.getNumericValue(this.getRepresentativeElement(fallRepStates))<=this.getNumericValue(fmaxState);
					assert this.getNumericValue(fminState)<=this.getNumericValue(fmaxState);
