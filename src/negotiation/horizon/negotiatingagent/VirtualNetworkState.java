...
public class VirtualNetworkState extends SimpleAgentState implements
	...
	Iterator<LinkMeasurableParameters> itLinksPref = linksPreferenceParams
		.iterator();
	for (OrderedPair<Integer> link : links) {
		...
		NetworkLinkParameters test = linksMap.add(
	...
	for (int i = 0; i < nbnodes; i++) {
	    ...
	    Map<VirtualNodeIdentifier, LinkAllocableParameters> ifacesAllocable = new HashMap<VirtualNodeIdentifier, LinkAllocableParameters>();
	    Map<VirtualNodeIdentifier, LinkMeasurableParameters> ifacesMeasurable = new HashMap<VirtualNodeIdentifier, LinkMeasurableParameters>();
	...
	for (Map.Entry<VirtualNodeIdentifier, VirtualNode> entry : initial.nodes
		.entrySet())
	...
	private boolean isValid() {
		...
		SubstrateNodeIdentifier peerHost = VirtualNetworkState.this.nodes
	...
	private Double evaluatePacketLossRateUtility() {
		...
		Interval<Float> demandedPacketLossRate = obtainedPacketLossRate
	...
	private Double evaluateDelayUtility() {
		...
		Interval<Integer> satisfyingDelay = obtainedDelay
	...
	private Double evaluateJitterUtility() {
	    LightAverageDoubleAggregation aggreg = new LightAverageDoubleAggregation();
		...
		Interval<Integer> satisfyingJitter = obtainedJitter
	...
	private Double evaluateAvailabilityUtility() {
	    ...
	    Interval<Integer> satisfyingAvailability = obtainedAvailability
	...
	LightAverageDoubleAggregation[][] aggregs = {
