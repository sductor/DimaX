...
public class InterfacesParameters<Identifier extends HorizonIdentifier, Parameters extends LinkParameters>
	...
	str.append("(");
	for (Parameters params : this.map.values()) {
	    str.append(params + ", ");
	}
	...
	for (Map.Entry<Identifier, LinkAllocableParameters> entry : ifacesParams1.map
		.entrySet()) {
	...
	for (Map.Entry<Identifier, LinkAllocableParameters> entry : ifacesParams2.map
		.entrySet()) {
	    Identifier key = entry.getKey();
     ...
     */
    public Parameters get(Identifier key) throws UnexistingLinkException {
