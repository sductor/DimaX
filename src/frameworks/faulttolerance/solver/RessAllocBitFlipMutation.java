...
public class RessAllocBitFlipMutation extends Mutation {
		...
		for (int j = 0; j < p.m; j++){
			...
			for (Integer i : agentOrder){
				boolean allocated=((Binary) solution.getDecisionVariables()[p.getPos(i, j)]).bits_.get(0);
					double optimistAgentcharge=Math.max(p.getAgentMemorycharge(i), p.getAgentProcessorCharge(i));
				double mutProb= getMutationProbability(probability,allocated, i, j, p.currentCharges[j],optimistAgentcharge);
				if (PseudoRandom.randDouble() < probability) {
					((Binary) solution.getDecisionVariables()[i]).bits_.flip(j);
					p.currentCharges[j]+=allocated?-optimistAgentcharge:+optimistAgentcharge;
	...
	private double getMutationProbability(double probability, boolean allocated, int agent, int host, double hostCharge,double optimistAgentcharge){
		...
		if (!allocated){
			if (hostCharge+optimistAgentcharge>p.getHostMaxCharge(host))
				...
				double agentSoftCrit=Math.pow(p.getAgentCriticality(agent),1);
				return probability*Math.max(agentSoftCrit,(1-hostChargePercent));
			}
		} else {
			...
			if (hostCharge>p.getHostMaxCharge(host))
				...
				agentSoftCrit=Math.pow(p.getAgentCriticality(agent),2);				
			return 1;//probability*hostChargePercent;//Math.min(0,-agentSoftCrit);			
		}
	}
