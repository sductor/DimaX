...
import vieux.dcop_old.daj.SchedulerDefault;
import vieux.dcop_old.daj.awt.*;

public class Network extends Thread {

	// set of nodes, its size, current number of nodes in set, nr of active nodes
	private int setSize = 100;
	private AgentNode[] set = new AgentNode[setSize];
	...
	public void add(AgentNode node) {
		if (setNum == setSize) {
			AgentNode oldSet[] = set;
			setSize *= 2;
			set = new AgentNode[setSize];
	...
	// --------------------------------------------------------------------------
	public void terminate(AgentNode node) {
		activeNum--;
		scheduler.terminate();
	}
	...
	// --------------------------------------------------------------------------
	public AgentNode getNode(int i) {
